...
function AbaLogistica({ usuario }) {
  ...
  const [abaSelecionada, setAbaSelecionada] = useState('info'); // Começa na aba informações
  ...
  const carregarMinhasEntregas = async () => {
    ...
    try {
      // Diferente URLs baseado no tipo de usuário
      let url;
  ...
  const podeVerInfo = true; // Informações são públicas
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      {/* Abas de Navegação - Apenas para usuários logados */}
      {isLoggedIn && (
      ...
      {/* Conteúdo das Abas */}
      {!isLoggedIn ? (
        // Exibir apenas informações para usuários não logados
        ...
        <>
                  ...
                  {entregadores.map(entregador => (
                    <div key={entregador._id} className="border rounded-xl p-4 hover:shadow-lg transition">
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <p className="flex items-center gap-2"><span className="text-gray-500">📍</span> {entregador.provincia || 'Não informada'}</p>
                      </div>
          ...
          {/* ABA: Minhas Entregas - Apenas para usuários logados com permissão */}
          {abaSelecionada === 'minhas_entregas' && podeVerMinhasEntregas && (
            <div className="bg-white rounded-xl shadow-lg p-6">
              <h3 className="text-xl font-bold text-gray-800 mb-4 flex items-center gap-2">
                <span className="text-2xl">📋</span> Minhas Entregas
                      ...
                      <div key={entrega._id} className="border rounded-xl p-4 hover:shadow-md transition">
                        <div className="flex flex-col md:flex-row justify-between gap-3">
                          <div className="flex-1">
                            ...
                            {entrega.observacoes && (
                              <p className="text-sm text-gray-500"><strong>📝 Observações:</strong> {entrega.observacoes}</p>
                            )}
                            ...
                            {entrega.clienteId && (usuario?.tipo === 'entregador' || usuario?.tipo === 'vendedor') && (
                              <p className="text-sm text-green-600"><strong>👤 Cliente:</strong> {entrega.clienteId.nome} - {entrega.clienteId.telefone}</p>
                            )}
                            {entrega.produtoId && (
                              <p className="text-sm text-purple-600"><strong>📦 Produto:</strong> {entrega.produtoId.nome}</p>
                            )}
                          </div>
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-white rounded-2xl p-6 max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button onClick={() => setModalCadastro(false)} className="text-gray-400 hover:text-gray-600">✕</button>
                </div>
                ...
                <div className="flex gap-3 mt-6">
                  <button onClick={() => setModalCadastro(false)} className="flex-1 py-2 bg-gray-200 rounded-lg hover:bg-gray-300">Cancelar</button>
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-white rounded-2xl p-6 max-w-md w-full mx-4">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button onClick={() => setModalEntrega(false)} className="text-gray-400 hover:text-gray-600">✕</button>
                </div>
                <div className="space-y-3">
                  <select className="w-full border p-3 rounded-lg" value={entregaSelecionada.entregadorId} onChange={(e) => setEntregaSelecionada({...entregaSelecionada, entregadorId: e.target.value})}>
                    ...
                    {entregadores.map(e => <option key={e._id} value={e._id}>{e.nome} - {e.veiculo} - {e.provincia}</option>)}
                  </select>
                ...
                <div className="flex gap-3 mt-6">
                  ...
                  <button onClick={solicitarEntrega} disabled={loading} className="flex-1 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Solicitar</button>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-white rounded-2xl p-6 max-w-md w-full mx-4">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button onClick={() => setModalDetalhes(null)} className="text-gray-400 hover:text-gray-600">✕</button>
                </div>
                <div className="space-y-3">
                  <p><strong>Status:</strong> <span className={`px-2 py-1 rounded-full text-xs ${getStatusBadge(modalDetalhes.status).cor}`}>{getStatusBadge(modalDetalhes.status).texto}</span></p>
                  ...
                  {modalDetalhes.observacoes && <p><strong>Observações:</strong> {modalDetalhes.observacoes}</p>}
                  {modalDetalhes.entregadorId && <p><strong>Entregador:</strong> {modalDetalhes.entregadorId.nome} - {modalDetalhes.entregadorId.veiculo}</p>}
                  {modalDetalhes.clienteId && <p><strong>Cliente:</strong> {modalDetalhes.clienteId.nome} - {modalDetalhes.clienteId.telefone}</p>}
                  {modalDetalhes.dataEntrega && <p><strong>Data de Entrega:</strong> {new Date(modalDetalhes.dataEntrega).toLocaleString()}</p>}
                </div>
